...
import mssi from "../assets/mssi.png";
import cisss from "../assets/cisss.jpg";
import laval from "../assets/laval.png";
import prds from "../assets/prds.png";

const ThemesDesktop = ({ themes, pages, setValue }) =>
  themes
    .filter((theme) => theme !== "Explorateur de lieux") // Exclude Place Explorer
    .map((theme, i) => {
      // Get the current theme data
      const themeData = pages.find((page) => page.theme === theme);

      // If no theme data or no subcategories, skip
      if (!themeData || !themeData.subcategories) return null;
          ...
          {themeData.subcategories.map((subcategory, subIndex) => (
            <div key={subIndex} className="subcategory-group">
              <div className="subcategory-title">
                <Translate str={subcategory.title} />
              </div>

              {subcategory.pages.map((page, pageIndex) => (
                <span
                  key={`${i}-${subIndex}-${pageIndex}`}
                  onClick={() =>
...
// Define the ThemesMobile component with subcategories
      ...
      <div className="footer__content">
        ...
        <div className="footer__content-second-half">
          ...
          <div className="grid">
            ...
            <div className="column">
              <div className="column__title">
                <Translate str="Collaborators" />
              </div>
              <img src={mssi} key="MSSI" alt="MSSI" />
              <img src={laval} key="laval" alt="Ville de Laval" />
            </div>
            <div className="column">
              <div className="column__title" style={{ border: "0px" }}></div>
              <img src={prds} key="PRDS" alt="PRDS" />
              <img src={cisss} key="cisss" alt="cisss" />
            </div>
              ...
              <div className="column column--cities">
                ...
                {cities.map((city, index) => (
                  <a href={city.href} key={city.id}>
                    <img src={city.src} alt={city.alt} width="86" height="38" />
                  </a>
                ))}
              </div>
            )}
